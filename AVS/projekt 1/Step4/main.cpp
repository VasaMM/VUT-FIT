...
//####################################################################################################################//
  ...
  for (size_t i = 0; i < imageCount; i++)
  {
    ...
    evaluateLayer(imagePixels, layerSize, &input[i * imagePixels], weight1, bias1, &output1[layerSize * i]);
  }
  
  for (size_t i = 0; i < imageCount; i++)
  {
    // The second layer 512 -> 512
    evaluateLayer(layerSize, layerSize, &output1[layerSize * i], weight2, bias2, &output2[layerSize * i]);
  }
  
  for (size_t i = 0; i < imageCount; i++)
  {
    // The third layer 512 -> 10
    evaluateLayer(layerSize, outputSize, &output2[layerSize * i], weight3, bias3, &output3[i*outputSize]);
  }
...
)
{
  ...
  const float* loop_weight = weight;

  for (size_t i = 0; i < neuronCount; i++) {
    output[i] = bias[i];

    #pragma omp simd simdlen(16) collapse(2) linear(input, loop_weight) reduction(+:output[i]) aligned(loop_weight:64, input:64)
    for (size_t j = 0; j < inputSize; ++j) {
      output[i] += loop_weight[j] * input[j];
    }
