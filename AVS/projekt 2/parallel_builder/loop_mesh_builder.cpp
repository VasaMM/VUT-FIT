...
unsigned LoopMeshBuilder::marchCubes(const ParametricScalarField &field) {
    ...
    // #pragma omp parallel for simd reduction(+:totalTriangles) ordered
    #pragma omp parallel for simd reduction(+:totalTriangles) schedule(static,32)
    for (size_t i = 0; i < totalCubesCount; ++i) {
        // 3. Compute 3D position in the grid.
        Vec3_t<float> cubeOffset(i % mGridSize, (i / mGridSize) % mGridSize, i / mGridSize_square);
