...
void const* gpu_computeVertexAttributeDataPointer(
	...
	return ( void const* )( ( char * ) head->buffer + head->offset + gl_VertexID * head->stride );
}
...
void gpu_runPrimitiveAssembly(
	...
	for ( size_t i = 0 ; i < nofPrimitiveVertices ; ++i ) {
	  ...
	  size_t tmpInvocation = baseVertexShaderInvocation + i;

	  gpu_runVertexPuller( &output, puller, tmpInvocation );
	  input.attributes  = &output;
	  input.gl_VertexID = gpu_computeGLVertexID( puller->indices, tmpInvocation );
...
// inline float distance_2D ( Vec2 const*const x, Vec2 const*const y ) {
float distance_2D ( Vec2 const*const x, Vec2 const*const y ) {
	return sqrtf (
	  powf ( x->data[0] - y->data[0], 2 ) + 
	  powf ( x->data[1] - y->data[1], 2 ) 
	);
}
...
void gpu_computeScreenSpaceBarycentrics(
	...
	float distance_AB = distance_2D( &vertices[0], &vertices[1] );
	float distance_BC = distance_2D( &vertices[1], &vertices[2] );

	// Jde nám jen o poměr -> není třeba obsah dělit dvěma
	float content_ABP = distance_AB * distanceTo2DLine( &lines[0], pixelCenter );
	float content_BCP = distance_BC * distanceTo2DLine( &lines[1], pixelCenter );
	float content_ABC = distance_AB * distanceTo2DLine( &lines[0], &vertices[2] );

	// Uložím spočítané souřadnice
	coords->data[2] = content_ABP / content_ABC;
	coords->data[0] = content_BCP / content_ABC;
	coords->data[1] = 1.0f - coords->data[2] - coords->data[0];
}
